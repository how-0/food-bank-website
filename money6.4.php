...
<head><title>Choose to edit user money donation</title></head>
...
<div class="form">
<form name="fSearch" id="fSearch" action="money6.4.php" method="POST">
...
<a href="money6.2.php" style="background-color: grey;color: white;padding: 10px 20px;text-align: center;text-decoration: none;display: inline-block;">Add Record</a>
</form>
...
if($key=="")
	...
	$sql = "SELECT * FROM tblmoney;";

else

	$sql="select * FROM tblmoney WHERE name LIKE '%$key%'OR amount LIKE '%$key%';";
...
?>
   ...
   Action:<br><a href="money6.5.php?id=<?php echo $row["id"]?>" style="background-color: grey;color: white;padding: 10px 20px;text-align: center;text-decoration: none;display: inline-block;">Edit</a> 
   <a href="money6.6.php?id=<?php echo $row["id"]?>"  style="background-color: grey;color: white;padding: 10px 20px;text-align: center;text-decoration: none;display: inline-block;">Delete</a> <br>
<?php
 }
} else {
 echo "0 results";
